...
_PIPELINE_PREV_RANK = None
_PIPELINE_NEXT_RANK = None
...
def initialize_model_parallel(
    ...
    num_tensor_group_belong: int = (rank // tensor_model_parallel_size)
    num_pipeline_group_belong: int = (rank % tensor_model_parallel_size)
    ...
    global _PIPELINE_PREV_RANK
    global _PIPELINE_NEXT_RANK
    if num_tensor_group_belong != 0:
        _PIPELINE_PREV_RANK = rank - tensor_model_parallel_size
    if num_tensor_group_belong != pipeline_model_parallel_size - 1:
        _PIPELINE_NEXT_RANK = rank + tensor_model_parallel_size
...
def get_pipeline_model_parallel_next_rank():
    ...
    assert _PIPELINE_NEXT_RANK is not None, (
        "Pipeline parallel group is not initialized")
    return _PIPELINE_NEXT_RANK


def get_pipeline_model_parallel_prev_rank():
    """Return the global rank that preceeds the caller in the pipeline"""
    assert _PIPELINE_PREV_RANK is not None, (
        "Pipeline parallel group is not initialized")
    return _PIPELINE_PREV_RANK
...
def is_tensor_model_parallel_first_rank():
