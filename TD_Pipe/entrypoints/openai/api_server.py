...
from TD_Pipe.engine.metrics import add_global_metrics_labels
from TD_Pipe.entrypoints.openai.protocol import (
...
from TD_Pipe.utils import random_uuid
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="TD-Pipe OpenAI-Compatible RESTful API server.")
...
async def create_chat_completion(request: ChatCompletionRequest,
    ...
    NOTE: Currently we do not support the following features:
        ...
        - logit_bias (to be supported by TD-Pipe engine)
    """
    ...
    if request.logit_bias is not None and len(request.logit_bias) > 0:
        # TODO: support logit_bias in TD-Pipe engine.
...
async def create_completion(request: CompletionRequest, raw_request: Request):
    ...
    NOTE: Currently we do not support the following features:
        - suffix (the language models we currently support do not support
          suffix)
        - logit_bias (to be supported by TD_Pipe engine)
    """
    ...
    if request.logit_bias is not None and len(request.logit_bias) > 0:
        # TODO: support logit_bias in TD_Pipe engine.
        return create_error_response(HTTPStatus.BAD_REQUEST,
                                     "logit_bias is not currently supported")
